# 🛡️ SOC Analyst Portfolio — onfroycmd

![Status](https://img.shields.io/badge/Status-W%20trakcie%20nauki-00d4ff?style=flat-square)
![Goal](https://img.shields.io/badge/Cel-Junior%20SOC%20Analyst%20%7C%202026-a855f7?style=flat-square)
![Platform](https://img.shields.io/badge/Platforma-TryHackMe%20%7C%20HackTheBox-00ff9f?style=flat-square)

> No degree. No bootcamp. Just me, a keyboard and a goal — Junior SOC Analyst.

---

## 📂 Struktura repo

| Folder | Co znajdziesz |
|--------|--------------|
| [`notes/`](./notes/) | Notatki z teorii — sieci, OS, HTTP, DNS |
| [`labs/`](./labs/) | Praktyczne ćwiczenia z narzędzi SOC |
| [`ctf-writeups/`](./ctf-writeups/) | Rozwiązania wyzwań TryHackMe / HackTheBox |
| [`incident-reports/`](./incident-reports/) | Symulowane raporty IR |
| [`scripts/`](./scripts/) | Skrypty Python/Bash do automatyzacji |

---

## 🧠 Co już umiem

- Sieci: TCP/IP, DNS, HTTP/S, nagłówki, kody odpowiedzi
- OS: kernel, przestrzeń użytkownika, uprawnienia, procesy
- Infrastruktura: Load Balancer, WAF, CDN, wirtualizacja, chmura
- Narzędzia: TryHackMe, HackTheBox

---

## 🗺️ Plan nauki (12 miesięcy)

- [x] Podstawy sieci i HTTP
- [x] Podstawy OS i wirtualizacji
- [ ] Wireshark — analiza ruchu sieciowego
- [ ] Splunk — SIEM, wykrywanie alertów
- [ ] Analiza malware (statyczna)
- [ ] TryHackMe SOC Level 1
- [ ] Google Cybersecurity Certificate
- [ ] CompTIA Security+

---

## 📫 Kontakt

GitHub: [onfroycmd](https://github.com/onfroycmd)
